const handler = async (m, {conn, text, participants, isOwner, usedPrefix, command}) => {
  ...
  const fproducto = {key: {fromMe: false, participant: `0@s.whatsapp.net`, ...(false ? {remoteJid: '5213339577315@s.whatsapp.net'} : {})}, message: {'productMessage': {'product': {'productImage': {'mimetype': 'image/jpeg', 'jpegThumbnail': banner}, 'title': `𝘊𝘖𝘔𝘜𝘕𝘐𝘊𝘈𝘚𝘐𝘖𝘕 𝘎𝘌𝘕𝘌𝘙𝘈𝘓`, 'description': botname, 'currencyCode': 'USD', 'priceAmount1000': '1000000000', 'retailerId': 'Ghost', 'productImageCount': 1}, 'businessOwnerJid': `0@s.whatsapp.net`}}};
  if (!m.quoted) throw m.reply(`✐ Responde un mensaje con el comando *${usedPrefix + command}* para mandar el aviso.`);
