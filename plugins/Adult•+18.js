...
sourceUrl: enlace }}})
   switch (command) {
    case 'pack':
      ...
      conn.sendMessage(m.chat, {image: {url: url}, caption: `_✐ Pack_`}, {quoted: m});
      break;
    case 'pack2':
      const url2 = await packgirl[Math.floor(Math.random() * packgirl.length)];
      conn.sendMessage(m.chat, {image: {url: url2}, caption: `_✐ Pack 2_`}, {quoted: m});
      break;
    case 'pack3':
      const url3 = await packmen[Math.floor(Math.random() * packmen.length)];
      conn.sendMessage(m.chat, {image: {url: url3}, caption: `_✐ Pack 3_`}, {quoted: m});
      break;
