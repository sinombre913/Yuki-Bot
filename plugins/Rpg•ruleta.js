...
let handler = async (m, { conn, text, command, usedPrefix }) => {
  ...
  if (coin > 50) return m.reply(`✐ La cantidad máxima de apuesta es de 50 *${moneda}*.`)

  if (!(color === 'black' || color === 'red')) return conn.reply(m.chat, "✐ Debes apostar a un color válido: *black* o *red*.", m)

  if (coin > users.coin) return m.reply(`✐ No tienes suficientes *${momeda}* para realizar esa apuesta.`)
  ...
  setTimeout(() => {
    ...
    if (win) {
      users.coin += coin
      m.reply(`✐ ¡Ganaste! Obtuviste ${coin} *${moneda}*. Total: ${users.coin} *${moneda}*.`)
    } else {
      users.coin -= coin
      m.reply(`✐ Perdiste. Se restaron ${coin} *${moneda}*. Total: ${users.coin} *${moneda}*.`)
    }
