...
let fkontak = { "key": { "participants":"0@s.whatsapp.net", "remoteJid": "status@broadcast", "fromMe": false, "id": "Halo" }, "message": { "contactMessage": { "vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:y\nitem1.TEL;waid=${m.sender.split('@')[0]}:${m.sender.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD` }}, "participant": "0@s.whatsapp.net" }
  if (!text) return conn.reply(m.chat, `[❗] Ponga el nombre de su video o musica  \n\n ejemplo: \n *${usedPrefix + command} hades bot md`, fkontak, m) 
  try {
 conn.reply(m.chat, `${wait}`, fkontak, m) 
    const yt_play = await search(args.join(' '));
    let additionalText = '';
    if (command === 'play3' || command == 'yta' || command == 'ytaudio' || command == 'audio' || command =='sonido') {
    additionalText = 'audio 🔊';
    } else if (command === 'play4' || command == 'ytv' || command == 'ytvideo' || command == 'video' || command == 'peliculas' || command == 'pelis') {
...
//conn.sendMessage(m.chat, {image: {url: yt_play[0].thumbnail}, caption: texto1}, {quoted: m});
  if (command == 'play3' || command == 'yta' || command =='sonido') {
    const q = '128kbps';
...
if (command == 'ytaudio' || command == 'audio') {
    const q = '128kbps';
    const v = yt_play[0].url;
    const yt = await youtubedl(v).catch(async (_) => await youtubedlv2(v));
    const dl_url = await yt.audio[q].download();
...
if (command == 'ytv' || command == 'ytvideo' || command == 'peliculas' || command == 'pelis') {
    const qu = '360';
    const q = qu + 'p';
    const v = yt_play[0].url;
    const yt = await youtubedl(v).catch(async (_) => await youtubedlv2(v));
    ...
    console.log(error)
    throw '*❌ lo siento ha corrido un error verifique que su enlace o el de YouTube sea correcto\n\n en caso de que no mande tu video o música es porque pesa demasiado el contenido recomendación para descargarlo usar\n\nplay3 y play4❌*';
  }
};
handler.help = ['play', 'play2'].map((v) => v + ' < busqueda >');
handler.help = ['pla3', 'play4'].map((v) => v + ' < busqueda >');
handler.tags = ['downloader'];
handler.command = ['play3', 'play4', 'yta', 'ytaudio', 'video', 'ytvideo', 'ytv', 'audio', 'sonido', 'peliculas', 'pelis']
