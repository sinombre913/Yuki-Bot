...
async function dansyaytdl(link) {
    try {
        const response = await axios.get('https://api.host-bot.store/api/dl/yt2/token');
        ...
        const url = `https://api.host-bot.store/api/dl/yt2?url=${encodeURIComponent(link)}&apikey=142d875c`;
        const headers = {
